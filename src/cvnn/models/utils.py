    ...
    if is_real_mode(layer_mode):
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Real loss '{real_loss_name}' not found in torch.nn or cvnn.losses")
    else:
...
def get_softmax(
    gumbel_softmax_type: str, layer_mode: str = "complex"
) -> Union[GumbelSoftmax, GumbelSoftmaxMeanCtoR, GumbelSoftmaxProductCtoR]:
    ...
    Returns:
        Union[GumbelSoftmax, GumbelSoftmaxMeanCtoR, GumbelSoftmaxProductCtoR]: Instantiated Gumbel softmax layer
...
def get_downsampling(
    ...
    if downsampling == "maxpool":
        ...
        return (
            ...
            if is_real
            else c_nn.AvgPool2d(kernel_size=factor, stride=factor, count_include_pad=False)
        )
...
def downsampling_lpd(
    ...
    if no_antialias == False:
        antialias_layer = partial(
            LowPassFilter, filter_size=3, padding="same", padding_mode="circular", layer_mode=layer_mode
        )
    return set_pool(
        partial(
            PolyphaseInvariantDown2D,
...
def upsampling_lpu(in_channels, gumbel_softmax_type, upsampling_factor, no_antialias, layer_mode):
...
def get_projection(    
    ...
    if is_real or projection == None:
        ...
        return MLPCtoR(hidden_sizes=hidden_sizes, input_size=input_size, output_size=output_size)
...
def get_dropout(
    dropout_prob: float, layer_mode: str, inplace: bool = False, use_2d: bool = True
) -> Optional[nn.Module]:
    ...
    if use_2d:
        ...
        if is_real_mode(layer_mode):
            # Use standard PyTorch dropout for real modes
            return nn.Dropout(p=dropout_prob, inplace=inplace)
        else:
