...
class GumbelSoftmaxMeanCtoR(nn.Module):
    ...
    def forward(self, x, tau, hard):
        return (
            (self.gumbel_softmax(x.real, tau, hard) + self.gumbel_softmax(x.imag, tau, hard)) / 2
        )
...
class GumbelSoftmaxProductCtoR(nn.Module):
    ...
    def forward(self, x, tau, hard):
        return (
            self.gumbel_softmax(
                self.gumbel_softmax(x.real, tau, hard) * self.gumbel_softmax(x.imag, tau, hard),
                tau,
                hard,
            )
        )
...
class SoftmaxProductCtoR(nn.Module):
    ...
    def forward(self, x, dim=1):
        return self.softmax(
            self.softmax(x.real, dim) * self.softmax(x.imag, dim), dim
        )