...
from .lps_logit_layers import LPSLogitLayersV2
from .lps_utils import lps_downsampleV2
...
def set_pool(
    ...
    if pooling_layer.func.__name__ == "PolyphaseInvariantDown2D":
        ...
        assert pooling_layer.keywords["component_selection"].__name__ in [
            "LPS",
            "max_p_norm",
        ]
        ...
        # in_channels required for antialias
        p = pooling_layer(in_channels=p_ch, no_antialias=no_antialias, stride=stride)
...
def max_p_norm(x, p=2, prob=None, x_nofilt=None):
    if prob is None:
        # Compute max index
        if x_nofilt is not None:
            # Compute indices from unfiltered components
            norms = [
                torch.norm(c.reshape(c.shape[0], -1), p=p, dim=1) for c in x_nofilt
            ]
        else:
...
class LPS(nn.Module):
    ...
    def forward(self, x, mode="train", prob=None, x_nofilt=None):
...
def split_polyV1(x, stride, in_channels, num_components):
...
def split_polyV2_legacy(x, stride, in_channels, num_components):
    _x = nn.functional.pixel_unshuffle(x, stride)
    components = []
    for i in range(num_components):
...
def split_polyV2(x, stride, in_channels, num_components):
    _, _, h, w = x.shape
...
class PolyphaseInvariantDown2D(nn.Module):
    ...
    def __init__(
        ...
        projection=None,
        stride: int = 2,
        component_selection=fixed_component,
        pass_extras=False,
        in_channels=None,
        comp_fix_train=False,
        get_logits=None,
        antialias_layer=None,
        use_get_logits=True,
        no_antialias=False,
        comp_train_convex=False,
        comp_convex=False,
        logits_pad="circular",
        ...
        if comp_fix_train:
            assert not (comp_convex)
        elif comp_convex:
            assert not (comp_fix_train)
        ...
        if self.no_antialias:
            self.antialias_layer = None
        else:
            self.antialias_layer = antialias_layer
    ...
    def forward(self, x, ret_prob=False, prob=None):
        ...
        if self.pass_extras:
            ...
            _x, _p = self.component_selection(
                x=components, x_nofilt=components_nofilt, mode=component_mode, prob=prob
            )
        else:
            ...
            _x, _p = self.component_selection(
                x=components, x_nofilt=components_nofilt, prob=prob
            )
