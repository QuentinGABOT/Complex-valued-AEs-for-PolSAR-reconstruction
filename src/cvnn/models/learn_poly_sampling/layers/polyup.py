...
def unpool_multistage(x, x_layer, p_dict, scale_factor, unpool_layer):
    # Refs lookup table
    assert np.log2(scale_factor) % 1 == 0, (
        "Scale factor not a power of 2. "
...
# Set unpool
def set_unpool(unpool_layer, p_ch, stride, softmax=None, gumbel_softmax=None, no_antialias=False):
    if unpool_layer.func.__name__ == "PolyphaseInvariantUp2D":
        ...
        if unpool_layer.keywords["component_selection"].__name__ == "LPS_u":
            ...
            u = unpool_layer(stride=2, in_channels=p_ch, no_antialias=no_antialias)
    else:
        raise ValueError(
            "Undefined unpooling layer. Check 'unpool_method' input argument."
        )
    return u
...
class PolyphaseInvariantUp2D(nn.Module):
    ...
    def forward(self, x, ret_prob=False, prob=None):
        ...
        if self.pass_extras:
            ...
            _x, _p = self.component_selection(x=x, mode=component_mode, prob=prob)
        else:
            # Max_p_norm_u
            # Pass pre-computed max index and components
            _x, _p = self.component_selection(x=x, prob=prob)
