...
def lps_downsampleV2(
    ...
    if mode in ["train", "test_gumbel"]:
        if polyphase_logits is None:
            ...
            # print("gumbel_softmax:", gumbel_softmax)
            # input()
            # _sampled_prob = F.gumbel_softmax(polyphase_logits, tau=tau, hard=hard)
            _sampled_prob = gumbel_softmax(polyphase_logits, tau=tau, hard=hard)
            # print("Sampled prob:", _sampled_prob)
            # input()
        sampled_prob = _sampled_prob.view(batch_size, 1, 1, 1, phase_size)
        ret = torch.sum(xpoly_combined * sampled_prob, -1, dtype=xpoly_combined.dtype)
    elif mode == "test":
        if polyphase_logits is None:
        ...
        # Keep max phase
        _, sampled_idx = torch.max(
            softmax(polyphase_logits), -1
        )  # we use the softmax in case polyphase_logits is of type complex.64
        ret = xpoly_combined[torch.arange(batch_size), :, :, :, sampled_idx]
        ...
        if polyphase_logits is None:
            ...
            # Compute weights
            _sampled_prob = F.softmax(polyphase_logits / tau, dim=-1)
        sampled_prob = _sampled_prob.view(batch_size, 1, 1, 1, phase_size)
        ret = torch.sum(xpoly_combined * sampled_prob, -1)
...
def lps_upsample(
    x, stride, polyphase_logits=None, mode="train", tau=1.0, hard=False, prob=None
):
...
def lps_upsampleV2(
    x, prob, stride, softmax, gumbel_softmax, mode="train", get_samples=False, tau=1.0, hard=False
):
    """LPS Upsampling Layer
    Args:
    ...
    if mode == "train":
        if get_samples:
            ...
            sampled_prob = gumbel_softmax(prob[1], tau=tau, hard=hard) #need to add gumbel_softmax
        else:
        ...
        sampled_prob = F.softmax(prob / tau, dim=-1)
        prob_poly = sampled_prob.repeat([1, cc]).view(bb, cc * stride**2, 1, 1)
        x_poly_weighted = x_poly * prob_poly
