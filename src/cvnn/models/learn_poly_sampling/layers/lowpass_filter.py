...
class LowPassFilter(nn.Module):
    def __init__(self, in_channels, filter_size, padding, padding_mode, layer_mode, filter_scale=1):
        super().__init__()
    ...
    def extra_repr(self):
        return (
            "in_channels={in_channels}, filter_size={filter_size}, padding={padding}, "
            "padding_mode={padding_mode}".format(
    ...
    def forward(self, inp):
        ...
        if self.layer_mode == "real":
            return F.conv2d(inp, filt, groups=inp.shape[1])
        elif self.layer_mode == "complex":
...
class DDAC(nn.Module):
    def __init__(
        self, in_channels, kernel_size, pad_type="reflect", group=2, kernel_scale=1
    ):
    ...
    def forward(self, x):
        ...
        x = (
            x.permute(1, 0, 2, 3)
            .reshape(self.group, c1 // self.group, n, p, q)
            .permute(2, 0, 1, 3, 4)
        )
        ...
        sigma = (
            ...
            .reshape(
                (
                    p // (self.kernel_size * self.kernel_size),
