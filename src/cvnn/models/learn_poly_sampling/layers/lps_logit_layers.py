...
class LPSLogitLayersV1(nn.Module):
    def __init__(self, in_channels, hid_channels, padding_mode):
        ...
        self.relu = c_nn.modReLU()  # need to be use the config file activation function
...
class LPSLogitLayersV2(nn.Module):
    ...
    def forward(self, x):
        ...
        ret = self.projection(self.conv(xpoly_combined))
        ret = ret.mean(dim=(-1, -2, -3))
        ret = torch.stack(torch.split(ret, batch_size), -1)
...
class SAInner(nn.Module):
    def __init__(self, in_channels, hid_channels, padding_mode, bn=False):
        ...
        self._phi = nn.Sequential(
            ...
            _bn(hid_channels),
            nn.Conv2d(
                hid_channels, hid_channels, 3, padding=1, padding_mode=padding_mode
            ),
        )
    ...
    def forward(self, x):
        b = x[0].shape[0]
        p = len(x)
        xpoly_combined = torch.flatten(
            x, start_dim=0, end_dim=1
        )  # torch.cat(x, dim=0)  # [BP x C x H x W]

        # Affinity
        phi = (
            self._phi(xpoly_combined).mean(dim=(-1, -2)).view(p, b, -1).transpose(0, 1)
        )  # [B x P x C]
        psi = (
            self._psi(xpoly_combined).mean(dim=(-1, -2)).view(p, b, -1).permute(1, 2, 0)
        )  # [B x C x P]
...
class GraphLogitLayers(nn.Module):
    def __init__(self, in_channels, hid_channels, padding_mode):
        ...
        self.input_conv = nn.Sequential(
            nn.Conv2d(
                in_channels, hid_channels, 3, padding="same", padding_mode=padding_mode
            ),
            nn.ReLU(inplace=True),
        )
    ...
    def setup_send_receive_matrix(self, phase_size, device):
        if phase_size != self.phase_size:
            ...
            self.receive_mat = torch.nn.functional.one_hot(
                torch.where(adj_mat)[1]
            ).float()
    ...
    def edge2node(self, x):
        ...
        return msg / (1.0 * self.receive_mat.size(-1))
    ...
    def forward(self, x):
        ...
        xpoly_combined = torch.flatten(x, start_dim=0, end_dim=1)
        xpoly_combined = (
            self.input_conv(xpoly_combined)
            .mean(dim=(-1, -2))
            .view(phase_size, batch_size, -1)
            .transpose(0, 1)
        )
