...
class BaseLinear(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = c_nn.Dropout(p=0.75)  # Default dropout, can be overridden
...
class LinearBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        layers = [BaseLinear(
            in_ch=in_ch, out_ch=out_ch, bias=bias, linear_mode=linear_mode
        )]
        
        # Use mode-aware normalization for linear layers
        # if normalization == "batch":
        #     # For linear layers, we need to handle batch norm differently
        #     if linear_mode == "real":
        #         normalization_layer = nn.BatchNorm1d(num_features=out_ch)
        #     else:
        #         normalization_layer = c_nn.BatchNorm1d(num_features=out_ch)
        # elif normalization == "layer":
        #     normalization_layer = get_normalization(
        #         norm_type=normalization,
        #         num_features=out_ch,
        #         layer_mode=linear_mode,
        #         normalized_shape=(out_ch,)
        #     )
        # else:
        #     normalization_layer = nn.Identity()
        normalization_layer = get_normalization(
            norm_type=normalization, num_features=out_ch, layer_mode=linear_mode
        )
        layers += [
            normalization_layer,
        ]
