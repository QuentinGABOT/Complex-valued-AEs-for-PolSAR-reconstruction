...
class ModelConfigValidator:
    ...
    def validate_autoencoder_config(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Required parameters
        required_params = ['num_channels', 'num_layers', 'channels_ratio', 'input_size', 'activation']
        ...
        if validated['activation'] not in valid_activations:
            warnings.warn(f"Activation '{validated['activation']}' may not be supported. "
        ...
        if validated['input_size'] < min_size:
            warnings.warn(f"input_size ({validated['input_size']}) might be too small for "
    ...
    def get_model_memory_estimate(config: Dict[str, Any]) -> Dict[str, float]:
        ...
        activation_memory_mb = input_size * input_size * num_channels * 4 / (1024 * 1024)
