...
def compute_class_weights(
    ...
    # Count frequencies
    counts = torch.bincount(valid.flatten(), minlength=num_classes).float().clamp(min=1.0)
    ...
    if weight_mode == "inverse_frequency":
        ...
        C = num_classes - (1 if 0 <= ignore_index < num_classes else 0)
        w = N / (C * counts)
    ...
    norm_C = num_classes - (1 if 0 <= ignore_index < num_classes else 0)
    w = w * norm_C / w.sum()
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_class_weights and self.weight is None:
            class_w = compute_class_weights(
                targets, C, device=inputs.device,
        ...
        # apply class‐weights inside CE?
        if class_w is not None:
            ce = F.cross_entropy(
                logits, t, weight=class_w,
                ignore_index=self.ignore_index,
                reduction='none'
            )[valid]
...
# Export focal loss classes for easy importing
__all__ = [
    "FocalLoss",
