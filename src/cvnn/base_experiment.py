...
class BaseExperiment(ABC):
    ...
    def __init__(
        ...
        validate_required_config_sections(self.cfg)
        ...
        self.full_loader, self.nsamples_per_rows, self.nsamples_per_cols = (
        ...
        layer_mode = getattr(self.model, "layer_mode", "complex")
        self.model.apply(lambda m: init_weights_mode_aware(m, layer_mode))
        self.model.to(self.device)
        # setup loss and optimizer (for retraining resume)
        self.loss_fn, self.optimizer = setup_loss_optimizer(self.model, self.cfg)
    ...
    def _setup_gumbel_tau(self) -> None:
        ...
        downsampling_layer = self.cfg.get("model", {}).get("downsampling_layer")
        if downsampling_layer not in ["LPD", "LPD_F"]:
            self.gumbel_tau_config = None
            return
            
        # Check if gumbel_tau config exists
        gumbel_config = self.cfg.get("model", {}).get("gumbel_tau")
        ...
        self.current_gumbel_tau = torch.tensor(gumbel_config["start_value"], dtype=torch.float32)
    ...
    def _initialize_gumbel_tau(self, tau: torch.Tensor) -> None:
        ...
        if not hasattr(self.model, 'encoder'):
            logger.warning("Model does not have encoder attribute for Gumbel tau initialization")
            return
        ...
        for enc in self.model.convnet.encoder[1:]:
            if hasattr(enc, 'down') and hasattr(enc.down, 'component_selection'):
                ...
                logger.warning(f"Encoder block missing expected structure for Gumbel tau: {enc}")
    ...
    def _setup_projection_state(self) -> None:
        ...
        self.projection_config = {
            "type": projection_layer,
            "config": projection_config
        }
        
        logger.info(f"Initialized projection state management: {projection_layer} with config {projection_config}")
